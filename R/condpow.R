...
#' 
#' @usage
#' condpow(time, status, rx, nsamp = 500, crit.val = 1.96, control = 0, 
#'         control.rate, test.rate, inf.time = 1, total.inf, add.acc = 0, 
#'         add.acc.period, p.con = .5, strat = rep(1,length(time)))
#' 
#' 
...
#' ## current data
#' set.seed(3)
#' ft <- c(rexp(100), rexp(100) / .95)
#' ct <- runif(200) * 3
#' fi <- ifelse(ct < ft, 0, 1)
#' ft <- pmin(ft, ct)
#' rx <- c(rep(0, 100), rep(1, 100))
#' 
#' ## currently at .43 information -- assume no prior interim analyses
#' critv <- sequse(c(.43,.7,1))[-1]
#' 
#' condpow(ft,fi,rx,nsamp=10,crit.val=critv,control.rate=1,test.rate=.75,
#'         inf.time=c(.7,1),total.inf=300,add.acc=200,add.acc.period=1)
#' 
#' #use larger nsamp in practice, eg
#' \dontrun{
#' condpow(ft,fi,rx,nsamp=1000,crit.val=critv,control.rate=1,test.rate=.75,
#'         inf.time=c(.7,1),total.inf=300,add.acc=200,add.acc.period=1)}
#' 
#' @export
...
condpow <- function(time,status,rx,nsamp=500,crit.val=1.96,
                    ...
                    add.acc=0,add.acc.period,p.con=.5,strat=rep(1,length(time))) {
  if (length(time) != length(status) | length(time) != length(rx))
    stop('invalid data')
  if (min(control.rate,test.rate,total.inf)<= 0) stop('invalid rates')
  if (length(crit.val) != length(inf.time)) stop('crit/inf')
  rx <- as.numeric(rx != control) + 1
  nd <- sum(status)
  if (nd>0) {
    z <- do.call('survdiff',list(formula=Surv(time,status)~rx+
      strata(strat),data=data.frame(time,status,rx,strat)))
    Uf <- sum(as.matrix(z$obs-z$exp)[1,])
    print(c(Logrank.score=Uf,Logrank.var=z$var[1,1]))
    z <- Uf/sqrt(z$var[1,1])
    cat('Observed Z = ',format(round(z,3)),'\n')
  } else {
    Uf <- 0
    cat('No events\n')
  }
  ...
  rho <- rho/sum(rho)
  fbar <- 1-nd/total.inf
  tmp <- sqrt(fbar*rho[1]*rho[2]*total.inf)
  b1 <- crv/sqrt(fbar)-Uf/tmp+tmp*log(test.rate/control.rate)
  b2 <- crv/sqrt(fbar)-Uf/tmp+sqrt(fbar*total.inf)*(test.rate-
    control.rate)/(test.rate+control.rate)
  d1 <- rho[1]/(rho[2]*test.rate/control.rate+rho[1])
  b3 <- b1*sqrt(d1*(1-d1)/(rho[1]*rho[2]))
  b4 <- (b1-crv/sqrt(fbar))*sqrt(d1*(1-d1)/(rho[1]*rho[2]))+crv/sqrt(fbar)
  cat('approx 1: ',format(round(1-pnorm(b1),3)),'\n')
  cat('Freedman: ',format(round(1-pnorm(b2),3)),'\n')
  cat('Exponent: ',format(round(1-pnorm(b3),3)),'\n')
  cat('Exponent2: ',format(round(1-pnorm(b4),3)),'\n')
  n2 <- nn <- length(time)
  ...
  if (add.acc>0) {
    ...
    if (length(ustrat)==1) strat <- c(strat,rep(ustrat,add.acc))
    else strat <- c(strat,rep(max(ustrat)+1,add.acc))
  }
  out2 <- rep(0,3)
  for (i in 1:nsamp) {
  ### generate data
    enter <- rep(0,nn)
    if (add.acc > 0) enter <- c(enter,runif(add.acc)*add.acc.period)
  ### ctime=calendar time of failures
    ...
    for (j in 1:length(crit.val)) {
      ...
      z <- do.call('survdiff',list(formula=Surv(newtime,newstat)~rx+
         strata(strat),data=data.frame(newtime,newstat,rx,strat)))
      zt <- sum(as.matrix(z$obs-z$exp)[1,])/sqrt(z$var[1,1])
  #(z$obs[1]-z$exp[1])/sqrt(z$var[1,1])
      if (zt>= crit.val[j]) tmp <- 1
    }
    out <- out+tmp
    out2 <- out2+c(sum(as.matrix(z$obs-z$exp)[1,])-Uf,
                   apply(as.matrix(z$obs),1,sum))
  }
  cat('Ave # failures at full information in control group:',
    format(out2[2]/nsamp),'\n')
  cat('Ave # failures at full information in experim. group:',
    format(out2[3]/nsamp),'\n')
  cat('Ave increment in logrank score:',format(out2[1]/nsamp),'\n')
