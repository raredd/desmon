...
#' 
#' @usage
#' surv1samp(control.rate = NULL, pct.imp = NULL, acc.per, add.fu,
#'           alpha = .1, power = .9, control.med = NULL,
...
#' 
#' @return A vector giving the sample size (n), the number of failures needed
...
#' @examples
#' surv1samp(-1.333*log(.96)/5,33.3,4,3,alpha=.1,power=.95)
#' surv1samp(-log(.95)/5,100*(log(.95)/log(.96)-1),4,3,alpha=.1,power=.95)
#' surv1samp(-log(.955)/5,100*(log(.955)/log(.965)-1),4,3,alpha=.1,power=.95)
#' 
#' @export surv1samp
...
surv1samp <- function(control.rate = NULL, pct.imp = NULL, acc.per, add.fu,
  ...
  if (!is.null(pct.imp)) {
    ...
    ratio <- control.rate/alt.rate
  } else stop('One of pct.imp, pct.reduc, alt.rate, alt.med, or alt.survp and tp must be specified')
  if (is.null(pct.imp)) pct.imp <- 100*(control.rate/alt.rate-1)
  # failure prob -- compute under alternative
  pcens <- (exp(-add.fu*alt.rate)-exp(-(add.fu+acc.per)*alt.rate))/
    (acc.per*alt.rate)
  pfail <- 1-pcens
  if (nonpar) {
    if (is.null(tp)) stop('tp must be specified for nonpar test')
    if (is.null(control.survp)) control.survp <- exp(-tp*control.rate)
    if (is.null(alt.survp)) alt.survp <- exp(-tp*alt.rate)
  # variance under the alternative
    if (tp<=add.fu) { # test hazard rate
      s1 <- exp(-tp*alt.rate)
      v <- (1-s1)/s1
    } else {
      if (tp>=add.fu+acc.per) stop('tp must be < add.fu+acc.per')
      s1 <- exp(-add.fu*alt.rate)
      f1 <- function(x) acc.per*alt.rate*exp(alt.rate*x)/(add.fu+acc.per-x)
      v <- integrate(f1,add.fu,tp)$value+(1-s1)/s1
    }
    n <- ((qnorm(1-alpha)+qnorm(power))/log(control.survp/alt.survp))^2*v
    nd <- n*pfail
  } else { # exponential
  # number events
