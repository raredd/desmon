...
twostg <- function(n1,n2,p1,r1,r2) {
  ...
  if (n1<1 | n2<1 | r1<0 | r2<0 | p1<=0 | r1>n1 | 
      r2>n2+n1 | p1>=1) stop('invalid arguments')
  x1 <- 0:n1
  x2 <- 0:n2
  w1 <- dbinom(x1,n1,p1)
  w3 <- dbinom(x2,n2,p1)
  ...
  u <- list(inputs=c(n1=n1,n2=n2,p1=p1,r1=r1,r2=r2),
...
#' 
#' @usage
#' simon(p0, pa, n1max = 0, ntmax = 1e+05, alpha = 0.1, beta = 0.1,
#'       del = 1, minimax = FALSE)
#' 
...
#' 
#' @return Returns a list with components \item{designs }{A matrix with a row
...
#' in \code{$designs}.}
#' 
#' @seealso \code{\link{twostg}}, \code{\link{bin1samp}},
#' \code{\link{pickwin}}, \code{\link{rp21}}
...
#' @examples
#' simon(.05, .20)
#' simon(.05, .20, minimax = TRUE)
#' 
#' @export simon

simon <- function(p0, pa, n1max = 0, ntmax = 1e5, alpha = .1, beta = .1,
                  del = 1, minimax = FALSE) {
  # prob calculations for 2-stage phase II design
  ...
  if (n1min>ntmax) stop('no valid designs')
  ...
  if (n1min<u1[1]) {
    if (n1max>0) n1max <- min(n1max,u1[1]-1) else n1max <- u1[1]-1
  } else if (n1min==u1[1]) {
    if (n1max>0) n1max <- min(n1max,u1[1]) else n1max <- u1[1]
  } else {
  ...
  if (n>ntmax) stop('no valid designs')
  e0 <- u1[1]
  ...
  # b0,ba=marginal dist of # responses H0, Ha
          ...
          if (ba[r2+1] <= beta) {
            ...
            if (minimax) {
              ...
              } else if (i+j == ntmax) e0 <- min(e0,e1)
            } else {
              e0 <- min(e0,e1)
            }
            z <- rbind(z,c(i,r1,j,r2,b0[r1+1],1-b0[r2+1],ba[r2+1],e1))
          } else {
            j <- j+1
            if (minimax) {
              if (i+j>ntmax) q <- 1 
            } else {
              if (e1>e0+del | i+j > ntmax) q <- 1
            }
          }
        }
      }
