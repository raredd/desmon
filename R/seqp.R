...
#' 
#' \code{eta} should be 0 to compute probabilites under the null, and should be
#' set to a value which is a function of the alternative on a transformed scale
#' to compute probabilties under the alternative.  For survival studies, eta
...
#' 
#' @return A \code{length(inf.times)} by 5 matrix with columns giving the
...
#' boundary.
#' 
#' @seealso \code{\link{sequse}}, \code{\link{seqopr}}, \code{\link{lr.inf}}
#' @keywords design
#' 
#' @examples
...
seqp <- function(inf.times, upper, lower = NULL, eta = 0) {
  ...
  kk <- length(inf.times) 
  if (is.null(upper)) upper=rep(6,kk) else upper=pmin(6,upper)
  if (is.null(lower)) lower=rep(-6,kk) else lower=pmax(lower,-6)
  if (kk>30) stop('too many analyses')
  if (length(upper) != kk | length(lower) != kk) stop('boundaries wrong length')
  u2 <- .Fortran('sqopr3',kk=as.integer(kk),pt=as.double(c(0,inf.times)),
                 eta=as.double(eta),uz=as.double(upper),uzl=as.double(lower),
                 pnu=double(kk+1),pnl=double(kk+1),
                 ierr=integer(1),PACKAGE="desmon")
  if (u2$ierr > 0) stop('Boundaries could not be computed')
  w2 <- cbind(u2$pt[-1],u2$uz,u2$uzl,diff(u2$pnu),diff(u2$pnl))
